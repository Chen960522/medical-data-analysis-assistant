...
import { dataService, type UploadProgress } from '../../services/dataService';
...
export interface FileUploadProps {
  ...
  maxSizeMb?: number;
  /** Called after a successful upload with the created file record. */
  onUploaded?: (file: DataFile) => void;
  /** Hint text rendered beneath the upload prompt. */
  hint?: string;
}
...
export function FileUpload({
  acceptExtensions = DEFAULT_DATA_EXTENSIONS,
  ...
  const doUpload = async (file: File) => {
    ...
    try {
      ...
      notify.success(
        `文件「${response.file.original_filename}」上传成功（${formatFileSize(
    ...
    <div>
      ...
      >
        ...
        <p className="ant-upload-hint">
          {hint ??
            `支持 ${acceptExtensions.join('、')} 格式，单个文件不超过 ${maxSizeMb}MB`}
        </p>
      </Dragger>
