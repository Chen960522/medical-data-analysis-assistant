...
import type { UploadProgress } from './dataService';
import type {
  DownloadFormat,
  DownloadMode,
  TranslateRequest,
...
export const translationService = {
  ...
  upload(
    ...
    signal?: AbortSignal,
  ): Promise<TranslationUploadResponse> {
    return new Promise<TranslationUploadResponse>((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            resolve(JSON.parse(xhr.responseText) as TranslationUploadResponse);
          } catch {
   ...
   */
  translate: (translationId: string, body?: TranslateRequest, signal?: AbortSignal) =>
    apiClient.post<TranslationResultResponse>(
      `/translation/${translationId}/translate`,
      body ?? {},
      { signal },
    ),

  /** Query translation progress + detected languages (Req 11.28). */
  getStatus: (translationId: string, signal?: AbortSignal) =>
    apiClient.get<TranslationStatusResponse>(`/translation/${translationId}/status`, { signal }),

  /** Fetch the bilingual Translation_Result (Req 11.30-11.34); 404 if not translated. */
  getResult: (translationId: string, signal?: AbortSignal) =>
    apiClient.get<TranslationResultResponse>(`/translation/${translationId}/result`, { signal }),

  /** Resolve a presigned download URL for the exported document (Req 11.37-11.41). */
  getDownloadUrl: (translationId: string, format: DownloadFormat, mode: DownloadMode) =>
    apiClient.get<TranslationDownloadResponse>(
      `/translation/${translationId}/download${buildQuery({ format, mode })}`,
    ),

  /** List the user's translation history, newest first (Req 11.43, 11.44). */
  getHistory: () => apiClient.get<TranslationHistoryResponse>('/translation/history'),

  /** Delete a translation record and its associated files (Req 11.46, 11.47). */
  deleteRecord: (translationId: string) =>
    apiClient.delete<void>(`/translation/${translationId}`),
};
