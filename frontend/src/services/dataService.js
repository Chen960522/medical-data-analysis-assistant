...
export const dataService = {
    ...
    listFiles: () => apiClient.get('/data/files'),
    /** Fetch the first-10-rows preview for a file (Req 2.2). */
    getPreview: (fileId) => apiClient.get(`/data/files/${fileId}/preview`),
    /** Fetch the data quality report for a file (Req 2.6). */
    getQuality: (fileId) => apiClient.get(`/data/files/${fileId}/quality`),
    deleteFile: (fileId) => apiClient.delete(`/data/files/${fileId}`),
};
